...
| 45 | Loader síncrono que lança derruba a **árvore React inteira** — tela branca, não aba com erro | no modo snapshot o carregador do JSON estoura quando o arquivo não existe. Lançado durante o render, o erro sobe até a raiz e não há boundary no caminho: a página some, não é a aba que fica vermelha | o loader devolve `{ error }` em vez de lançar, e cada aba renderiza a própria falha. Pergunta fora do conjunto medido vira **aviso explicado** — nunca lista vazia, que numa tela comparando motores de busca lê como "nenhum achou nada", a conclusão errada |
| 46 | Asserção de canário verde com o **painel vazio** na frente | a palavra-âncora era `vetor`, que existe só em **comentário de código** (`App.jsx:4` e `:60`) — em texto de tela nenhum. A asserção falhava com a página certa aberta, e passaria com a página errada | a âncora sai do conteúdo que só existe **depois** do `fetch`: `"cópia 2"`, título do segundo painel, renderizado apenas quando `fetchDocument` resolve. Âncora tirada de cabeçalho de aba tem o mesmo defeito — o cabeçalho está lá antes de qualquer dado chegar |
| 47 | `task web:publish` aborta exibindo um `"success": true` **como se fosse erro** | `resp \| grep -q` sob `set -euo pipefail`: o `grep -q` fecha o pipe ao casar, quem escreve leva SIGPIPE e o pipeline devolve **141**. A falha é **invertida** — o teste quebra exatamente quando a asserção é verdadeira. A forma grave é a mesma construção no scan de segredo do `16`: ela daria "limpo" no commit que TEM segredo | `grep -cE` (conta e lê o stream inteiro) no lugar de `grep -qE`, e `json.loads(sys.argv[1])` no lugar de `json.load(sys.stdin)`. Verificado invertendo: segredo enterrado no meio de 800 linhas, `1 achado` |
| 48 | A correção da 47 trocou o pipe por herestring e o script **travou** até o `timeout` matar (rc 143) | `<<<` materializa o conteúdo num arquivo em `$TMPDIR`, e aqui isso **pendura acima de ~100 B**. Medido: 100 B passa, 1 000 B trava, e uma resposta de projeto do Pages tem ~6 KB. Vale para `cat <<<` também, então não é do `python3` | argumento (`sys.argv[1]`, e 6 KB cabe folgado no ARG_MAX de 1 MB) ou pipe com leitor que consome tudo. **Nem `grep -q` em pipe, nem herestring**: as duas formas curtas de escrever a asserção falham, cada uma do seu jeito |
| 49 | O scan de segredo do `16` passa a **acusar o próprio script** | o canário `+AKIA0000000000CANARY` está versionado: o `git show` do commit que o introduziu casa a regex. Medido em `5874bf6`, 1 match. A varredura aborta a si mesma, e o falso positivo ensina a ignorar o alarme — que é como um verdadeiro passa | a isca é montada por concatenação (`printf '+%s%s' 'AKIA' '…'`), então o arquivo novo não se auto-acusa; e o histórico já publicado é filtrado por `grep -cv CANARY`. Filtrar pela palavra é honesto: ela existe para marcar a isca, e segredo de verdade não a traz |
| 50 | 12 commits publicados **sem assinatura**, com o `commit.gpgsign` ligado o tempo todo | `git subtree split` **reescreve** cada commit (árvore e pai diferentes): o SHA novo nasce sem assinatura, porque a original cobre a árvore do monorepo, não esta. Nada avisa — o `%G?` só é consultado se alguém pensar em consultar | reassinar antes do push, em worktree: `git worktree add` + `git rebase --root --exec 'git commit --amend --no-edit -S'`, e abortar se algum `%G?` != `G`. A branch está *checked out* no worktree, então a rebase já a move — `git branch -f` ali é recusado, e é o certo |
